...
void DistributedDijkstra::execute(GraphData &graph) {
  ...
  while (true) {
    ...
    MPI_CHECK(MPI_Allreduce(&local_min, &global_min, 1, MPI_FLOAT_INT,
    ...
    MPI_CHECK(MPI_Bcast(sendCounts.data(), rankSize, MPI_INT, globalRank,
                        MPI_COMM_WORLD));
    ...
    if (globalRank == rank) {
      ...
      for (int r = 0; r < rankSize; r++) {
        if (r != rank && sendCounts[r] > 0) {
          ...
          MPI_Request req;
          MPI_CHECK(MPI_Isend(outMsgs[r].data(), bytes, MPI_BYTE, r, 0,
                              MPI_COMM_WORLD, &req));
      ...
      MPI_CHECK(MPI_Recv(incMsgs.data(), sendCounts[rank] * sizeof(UpdateMsg),
                         MPI_BYTE, globalRank, 0, MPI_COMM_WORLD,
                         MPI_STATUS_IGNORE));
...
void DistributedDijkstra::reportMetrics() const {
  ...
  MPI_CHECK(MPI_Reduce(&m_numIterations, &globalIters, 1, MPI_INT, MPI_MAX, 0,
                       MPI_COMM_WORLD));
  ...
  MPI_CHECK(MPI_Reduce(m_distances.data(), globalDistances.data(),
                       m_distances.size(), MPI_FLOAT, MPI_MIN, 0,
                       MPI_COMM_WORLD));
