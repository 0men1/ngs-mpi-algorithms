...
void DistributedLeaderElection::execute(GraphData &graph) {
  ...
  for (int round = 0; round < m_rounds; round++) {
    ...
    MPI_CHECK(MPI_Alltoall(sendCounts.data(), 1, MPI_INT, recvCounts.data(), 1,
    ...
    MPI_CHECK(MPI_Alltoallv(sendBuf.data(), sendCounts.data(), sdispls.data(),
                            MPI_BYTE, recvBuf.data(), recvCounts.data(),
                            rdispls.data(), MPI_BYTE, MPI_COMM_WORLD));
...
void DistributedLeaderElection::reportMetrics() const {
  ...
  MPI_CHECK(MPI_Reduce(&m_numMessages, &globalMsgs, 1, MPI_INT, MPI_SUM, 0,
                       MPI_COMM_WORLD));
  MPI_CHECK(MPI_Reduce(&m_bytesSent, &globalBytes, 1, MPI_INT, MPI_SUM, 0,
                       MPI_COMM_WORLD));
  ...
  for (const auto &[node, leader] : m_finalLeaders) {
    if (leader < localMinLeader)
      localMinLeader = leader;
    if (leader > localMaxLeader)
      localMaxLeader = leader;
  }

  if (m_finalLeaders.empty()) {
    localMinLeader = std::numeric_limits<int>::max();
    localMaxLeader = -1;
  }

  int globalMinLeader = -1, globalMaxLeader = -1;
  MPI_CHECK(MPI_Reduce(&localMinLeader, &globalMinLeader, 1, MPI_INT, MPI_MIN,
                       0, MPI_COMM_WORLD));
  MPI_CHECK(MPI_Reduce(&localMaxLeader, &globalMaxLeader, 1, MPI_INT, MPI_MAX,
                       0, MPI_COMM_WORLD));
